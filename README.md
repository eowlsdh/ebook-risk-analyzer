...
- 그래픽 사용자 인터페이스(GUI)는 제공하지 않습니다. 아래 명령줄만 지원합니다.

## Windows 설치 (비개발자용)

1. [python.org/downloads/windows](https://www.python.org/downloads/windows/)에서 Python 3.11 이상 **Windows installer (64-bit)** 를 내려받아 설치합니다. 설치 첫 화면에서 **Add python.exe to PATH** 를 체크합니다.
2. 새 PowerShell을 열고 다음으로 확인합니다.

```powershell
py -3.11 --version
```

`py`가 없으면 `python --version`을 사용합니다. Python 3.11 이상이 표시되어야 합니다.
3. 프로젝트 폴더에서 가상 환경을 만들고 활성화합니다.
...
PowerShell이 스크립트를 차단하면 현재 창에서만 다음을 실행한 뒤 다시 활성화합니다.
...
## Windows 로컬 앱 (설치형/휴대용 배포물)
 
이 절은 **향후 릴리스 빌드가 제공하는** 설치 파일 또는 휴대용 폴더를 받은 비개발자용 안내입니다. 이 소스 체크아웃에는 완성된 `Setup.exe`나 휴대용 실행 폴더가 이미 들어 있다고 주장하지 않습니다. 실행 파일이 없고 이 저장소만 있다면 위의 Python 기반 설치 절차를 사용하십시오.
 
- 설치형 배포물은 게시자가 제공한 `Setup.exe`를 실행해 설치합니다. 설치 전 파일의 발급자·디지털 서명을 확인하고, 제거는 Windows **설치된 앱**에서 합니다.
